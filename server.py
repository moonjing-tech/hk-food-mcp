...
app = FastAPI(
    title="HongKong Food MCP & REST API",
    version="2.1.0",
)

mcp = FastMCP(
    "HongKong Food Database MCP Server",
    host="0.0.0.0",
    port=PORT,
)
...
async def api_search_food(
    keyword: str = Query(..., description="要查询的香港食品名称"),
):
    return search_food(
        supabase=supabase,
        query=keyword,
...
def filter_foods_by_nutrition(
    ...
    """按热量、蛋白质、钠等营养指标筛选食品。"""
    result = filter_foods_by_nutrition(
        supabase=supabase,
